...
public sealed class LegacyAuthAccountRepository : ILegacyAccountRepository
{
    ...
    private readonly LegacyAuthDbConnectionFactory _connectionFactory;

    public LegacyAuthAccountRepository(LegacyAuthDbConnectionFactory connectionFactory)
    {
    ...
    public async Task<AuthAccountRecord?> GetByUsernameAsync(string username, CancellationToken cancellationToken)
    {
        const string sql = """
            SELECT
                Id,
        ...
        return await connection.QuerySingleOrDefaultAsync<AuthAccountRecord>(new CommandDefinition(
            sql,
            new { Username = username },
            cancellationToken: cancellationToken));
    }
