    ...
    public class PushBack : SpellEffectHandler
    {
        ...
        public override void Apply()
        {
            ...
            foreach (var actor in GetAffectedActors().Where(x => x != null).OrderByDescending(x => x.Position.Point.DistanceToCell(targetedPoint)))
            {
                ...
                var point = TargetedCell == actor.Position.Cell ? new MapPoint(Caster.Position.Cell) : targetedPoint;
                if (point.CellId == actor.Position.Cell)
                    continue;

                var direction = point.OrientationTo(actor.Position.Point, false);
