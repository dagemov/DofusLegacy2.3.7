...
{
    [AIHandler(AIEnum.CRAZY)]
    public class Crazy : AITypeHandler
    {
        public override void Play()
        {
            var fighters = this.GetAllFighters(Fighter.Stats.MP.TotalMax, true).Where(x => !(x is ISummoned));

            var friends = this.GetAllFightersAdjacent(Fighter.Stats.MP.TotalMax, true);

            var fightersFarAway = this.GetAllFighters(Fighter.Stats.MP.TotalMax).Where(x => !(x is ISummoned));

            bool isCrazy = new AsyncRandom().Next(0, 2) == 1;
        ...
        private void Move(bool crazy = false)
        {
            ...
            FightActor fighter = GetFirstFighterWithLessHealth(GetAllFighters(Fighter.Stats.MP.TotalMax, true).Where(x => !(x is ISummoned)));
            ...
            for (int i = 0; i < Fighter.Stats.MP.TotalMax; i++)
            {
                ...
                if (fighter == null)
                {
                    fighter = GetFirstFighterWithLessHealth(GetAllFighters(Fighter.Stats.MP.TotalMax).Where(x => !(x is ISummoned)));
